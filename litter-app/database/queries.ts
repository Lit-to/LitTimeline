...
export const GET_TIMELINE_FROM_ID =
    "SELECT * FROM litter.posts WHERE user_id = ? and id < ? and is_deleted = false order by created_at DESC,id DESC LIMIT ?";
export const GET_TIMELINE_FROM_MAX = "SELECT * FROM litter.posts WHERE user_id = ? and is_deleted = false order by created_at DESC,id DESC LIMIT ?";
