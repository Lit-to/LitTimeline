 ...
 */
async function insertPost(userId: string, contents: string): Promise<QueryResult.QueryResult<boolean>> {
    try {
        await db.query(queries.INSERT_POSTS_CONTENTS, [userId, contents]);
