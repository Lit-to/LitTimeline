...
+-------------------------------------------------+--------+
```
今回はパスワードの最低要件を設定していないので、デフォルトのままとする。
この設定だと8文字以上の大文字、小文字、特殊文字を一文字以上含めないといけない。
...
-   ``dev@%``:開発者用ユーザー,SELECT,INSERT,UPDATE,DELETE,ALTER TABLE,CREATE TABLEを``litter``データベースに実行可能。
