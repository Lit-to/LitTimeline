...
# is_exist
## 1.まだidが存在していないパターン
result=$(curl -X POST http://localhost:3000/is_exist -H "Content-Type: application/json" -d '{"id": "lit_to"}')
...
# is_exist
## 8.既にidが存在しているパターン(テストとしては独立変数が2つあるので微妙な形になっているが割愛)
result=$(curl -X POST http://localhost:3000/is_exist -H "Content-Type: application/json" -d '{"id": "lit_to"}')
echo -e $result >> ./result.txt
